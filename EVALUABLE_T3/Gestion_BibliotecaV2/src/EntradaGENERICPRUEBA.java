...
import controller.BibliotecaGenericPRUEBA;
import controller.OperacionesFicheroObj;
import model.CatalogoLlenoException;
import model.CatalogoNoExisteException;
import model.NoExisteLibroEnBusqueda;
import model.TipoDatosNoContemplados;

import java.util.InputMismatchException;
import java.util.Scanner;

public class EntradaGENERICPRUEBA {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);

        BibliotecaGenericPRUEBA bibliotecaGeneral = null;
        BibliotecaGenericPRUEBA bibliotecaTerror = null;
        BibliotecaGenericPRUEBA bibliotecaPoliciaca = null;
        BibliotecaGenericPRUEBA bibliotecaComedia = null;

        OperacionesFicheroObj operacionesFicheroObj = new OperacionesFicheroObj();

        //BORRAR FICHERO Y CREARLO DE NUEVO
        // TODO: 15/04/2024 SI ME QUEDO ESTE; ACTIVAR
      ...
      //  operacionesFicheroObj.crearObjLibros();
        ...
        do {
            ...
            switch (tipoBiblioteca) {
                case 1:
                    bibliotecaGeneral = new BibliotecaGenericPRUEBA("Biblioteca Municipal", "Cooperativa");
                    System.out.println("🏛️Has creado una Biblioteca General🏛️\n");

                    break;
                case 2:
                    bibliotecaTerror = new BibliotecaGenericPRUEBA("Biblioteca Municipal Terror", "Cooperativa");
                    System.out.println("🏛️Has creado una Biblioteca de Novela Terror🏛️\n");

                    break;
                case 3:
                    bibliotecaPoliciaca = new BibliotecaGenericPRUEBA("Biblioteca Municipal Policiaca", "Cooperativa");
                    System.out.println("🏛️Has creado una Biblioteca de Novela Policiaca🏛️\n");

                    break;
                case 4:
                    bibliotecaComedia = new BibliotecaGenericPRUEBA("Biblioteca Municipal Comedia", "Cooperativa");
...
// TODO: 14/04/2024 IF opcion 2, si catalogo es !null  desaparece la opcion 2 XD MAGIA 
                switch (opcionMenuInterior) {
                    ...
                    case 3:
                        if (tipoBiblioteca == 1) {
                            bibliotecaGeneral.agregarLibroEnCatalogoGENERIC();
                        }
                        if (tipoBiblioteca == 2) {
                            bibliotecaTerror.agregarLibroEnCatalogoGENERIC();
                        }
                        if (tipoBiblioteca == 3) {
                            bibliotecaPoliciaca.agregarLibroEnCatalogoGENERIC();
                        }
                        if (tipoBiblioteca == 4) {
                            bibliotecaComedia.agregarLibroEnCatalogoGENERIC();
                        }
                        break;
                    case 4:
                        if (tipoBiblioteca == 1) {
