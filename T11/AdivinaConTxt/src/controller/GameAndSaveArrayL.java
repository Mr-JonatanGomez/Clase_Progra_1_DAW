...
public class GameAndSaveArrayL {
    ...
    // TODO: 05/05/2024 NO FUNCIONA ESTO 
    public void leerHistorialCopiarArrayYGuardarSiNombreIgual() {
        ArrayList<String> listadoLeer = new ArrayList<>();
        ArrayList<String> listadoEscribir = new ArrayList<>();
        //HACER SIEMPRE AL COMENZAR PARTIDA para IGUALAR RECORD
        File file = new File("src/resources/historial.txt");
        BufferedReader bufferedReader = null;
        PrintWriter printWriter = null;
        ...
        try {
            ...
            bufferedReader = new BufferedReader(new FileReader(file));
            String jugadorRecord = null;
            System.out.println("HISTORIAL");
            while ((jugadorRecord= bufferedReader.readLine()) != null) {
                //asi leemos mientras no se null, y guardamos a un listado
                listadoLeer.add(jugadorRecord);
            }
            for (String item : listadoLeer) {
               String [] jugYrec = item.split(" ");
               int pasarRecordInt = Integer.parseInt(jugYrec[1]);

                if (jugador.getNombre().equalsIgnoreCase(jugYrec[0]) && jugador.getPuntuacionPartidaActual()<pasarRecordInt){
                  // si es igual y mejora datos escribimos
                    pasarRecordInt=jugador.getPuntuacionPartidaActual();
                    String pasarRecordString = String.valueOf(pasarRecordInt);
                    jugYrec[1]=pasarRecordString;
                    item = jugYrec[0]+" "+jugYrec[1];
                    listadoEscribir.add(item);
                    break;

                } else if (jugador.getNombre().equalsIgnoreCase(jugYrec[0]) && jugador.getPuntuacionPartidaActual()>=pasarRecordInt) {
                    System.out.println("El jugador ya existia, pero no mejora su puntuacion");
                } else {
                    // si no es igual, escribimos
                    listadoEscribir.add(item);
                }
            }
        ...
        try {
            ...
            printWriter.write(String.valueOf(listadoEscribir));
        } catch (FileNotFoundException e) {
    ...
    public void partida() {
        ...
        //SI SE SUPERA EL RECORD PARTIDA INSCRIBIR RECORD
        ...
        //guardarHistorial(jugador);
        leerHistorialCopiarArrayYGuardarSiNombreIgual();
